...
const verifier_question = [
  ...
  },
  {
    ...
    name: 'file_name',
    message:
      'Enter the contract file name. (e.g. MultiSigWalletWithDailyLimit.sol)',
  },
  {
    type: 'input',
    name: 'contract_address',
    message:
      'Enter the contract address on blockchain. (e.g. 0x851b7f3ab81bd8df354f0d7640efcd7288553419)',
  },
  {
    type: 'input',
    name: 'is_optimized',
  ...
  .action((chainChoice) => {
    if (typeof chainChoice == 'undefined') {
      ...
      if (
        ...
        console.log(
          chalk.red.bold('Invalid chain choice') +
            ' Your current choice is by default: mainchain',
        )
        console.log(
          chalk.green.bold('Please choose from: ') +
            chalk.underline('mainnet') +
            ' , ' +
            chalk.underline('ropsten') +
            ' , ' +
            chalk.underline('kovan') +
            ' , ' +
            chalk.underline('rinkeby'),
        )
      }
    }
  })
...
program
  .command('compiler')
  .description(
    'Complete compiler version look up (major release plus all nightly commits)',
  )
  .action(() => {
